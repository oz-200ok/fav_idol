...
class PreferenceAPITests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.initial_subscription = UserGroupSubscribe.objects.create(
            user=cls.user, group=cls.group1, notification=True
        )
        # URL 정의
        cls.subscribe_list_create_url = reverse("subscribe-list")
        cls.subscribe_detail_url = lambda pk: reverse(
            "subscribe-detail", kwargs={"pk": pk}
        )
        cls.subscribed_schedules_url = reverse("user-subscribed-schedules")
        cls.schedule_detail_url = lambda pk: reverse(
            "user-schedule-detail", kwargs={"schedule_id": pk}
        )
    ...
    def test_subscription_crud_flow(self):
        ...
        create_response = self.client.post(
            self.subscribe_list_create_url, data=create_data
        )
        self.assertEqual(create_response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(create_response.data["data"]["group_id"], self.group2.id)
        new_subscription_id_from_response = create_response.data["data"][
            "id"
        ]  # 응답에서 id 확인 (삭제 시 사용 가능)
        ...
        self.assertEqual(
            list_response_after_delete.data["data"][0]["group_id"], self.group1.id
        )
